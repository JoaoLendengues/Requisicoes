"""
Importação de clientes da planilha relacao_cadastros.ods para o PostgreSQL.
Execute na raiz do projeto NO SERVIDOR:
    .venv\Scripts\python importar_clientes.py
...
COL_MAP = {
    ...
    "cpfcnpj":  "cpf_cnpj", "cnpj": "cpf_cnpj", "cpf": "cpf_cnpj",
...
def importar_clientes():
    ...
    for _, row in df.iterrows():
        ...
        cpf_cnpj = None
        if col_map.get("cpf_cnpj"):
            val = str(row.get(col_map["cpf_cnpj"], "")).strip()
            cpf_cnpj = val if not _is_blank(val) else None

        rows.append((code, name, cpf_cnpj))

    if not rows:
        print("❌ Nenhum registro válido encontrado na planilha.")
        return

    print(f"\n   {len(rows)} clientes prontos para importar  |  {skipped} linhas ignoradas")
    ...
    try:
        ...
        execute_values(
            cur,
            """
            INSERT INTO clients (code, name, cpf_cnpj, is_active, created_at, updated_at)
            VALUES %s
            ON CONFLICT (code) DO UPDATE
                SET name       = EXCLUDED.name,
                    cpf_cnpj   = COALESCE(EXCLUDED.cpf_cnpj, clients.cpf_cnpj),
                    updated_at = NOW()
            """,
            rows,
            template="(%s, %s, %s, TRUE, NOW(), NOW())",
        )
        conn.commit()
        print(f"✅ {len(rows)} clientes importados com sucesso  |  {skipped} linhas ignoradas")
