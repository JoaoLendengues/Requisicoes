...
def global_style() -> str:
    return (
        ...
        f"}}"
        f"QMessageBox {{"
        f"  background:{CARD_BG}; border:1px solid {BORDER_COLOR}; border-radius:8px;"
        f"}}"
        f"QMessageBox QWidget {{ background:{CARD_BG}; color:{TEXT_DARK}; }}"
        f"QMessageBox QLabel {{ background:transparent; color:{TEXT_DARK}; padding:2px 0; }}"
        f"QMessageBox QFrame {{ background:{CARD_BG}; border:none; }}"
        f"QMessageBox QDialogButtonBox {{ background:{CARD_BG}; border-top:1px solid {BORDER_COLOR}; padding-top:10px; }}"
        ...
        f"QInputDialog, QFileDialog {{"
        f"  background:{CARD_BG}; color:{TEXT_DARK};"
        f"}}"
        f"QInputDialog QLabel, QFileDialog QLabel {{"
        f"  background:transparent; color:{TEXT_DARK};"
        f"}}"
        ...
        f"  selection-background-color:{SELECTION_BG}; selection-color:{TEXT_DARK};"
        f"}}"
        f"QInputDialog QComboBox, QFileDialog QComboBox {{"
        f"  background:{INPUT_BG}; border:1px solid {BORDER_COLOR}; border-radius:8px;"
        f"  padding:7px 10px; color:{TEXT_DARK};"
        f"  selection-background-color:{SELECTION_BG}; selection-color:{TEXT_DARK};"
        f"}}"
        ...
        f"QInputDialog QDialogButtonBox, QFileDialog QDialogButtonBox {{"
        f"  background:{CARD_BG}; border-top:1px solid {BORDER_COLOR}; padding-top:10px;"
        f"}}"
        f"QInputDialog QPushButton, QFileDialog QPushButton {{"
        f"  background:{PRIMARY}; color:{TEXT_WHITE}; border:none;"
        f"  border-radius:8px; padding:7px 16px; min-width:84px; font-weight:600;"
        f"}}"
        f"QInputDialog QPushButton:hover, QFileDialog QPushButton:hover {{"
